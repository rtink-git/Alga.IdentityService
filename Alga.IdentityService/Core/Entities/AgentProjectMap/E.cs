namespace Alga.IdentityService.Core.Entities.AgentProjectMap;

public class E : Infrastructure.KVA.AdbABase<Guid?>, IE
{
    readonly ILogger<E>? _logger;
    public E(ILogger<E>? logger = null) : base("agent_project_map_table", logger)
    {
        _logger = logger;
    ...
    internal async Task<bool> AddAsync(Guid? guid, Guid? value)
    {
        ...
        if (value == Guid.Empty) throw new ArgumentException($"{_logAddAsync} - the 'guid' cannot be: {Guid.Empty}");
